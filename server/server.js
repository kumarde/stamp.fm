	...
	if (req.session.user == null && req.user == null && req.session.temp == null) {
		...
		dbt.findOne({_id:id}, function(e,p){
			...
			if (!p.elim){
				...
				res.render('elim', {temp: tacc,imgid: "0", v1id: p.elim.v1id, v2id:p.elim.v2id, song1: p.elim.v1name, song2: p.elim.v2name, user1: p.elim.v1artist, user2: p.elim.v2artist, votes1: p.elim.v1v, votes2: p.elim.v2v});
			}
		});
	}
});
